...
public class AddProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			diskFileItemFactory.setRepository(new File("E:\\temp\\"));
			...
			for (FileItem fileItem : list) {
				...
				if(fileItem.isFormField()){
					...
					// 文件上传项 
					// 获得文件名:
					fileName = fileItem.getName();//aaa.txt
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//SimpleDateFormat sdf = new SimpleDateFormat("", Locale.UK);  
	}

	public static void main(String[] args) {
		String name="132789.jpg";
		String fm=name.substring(name.indexOf(".")+1);
		System.out.println(name.substring(name.indexOf(".")+1));
		String format="jpgpng";
		if(format.contains(fm.toLowerCase())){
